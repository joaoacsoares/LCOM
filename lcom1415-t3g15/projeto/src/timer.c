...
#define BITORDER_TIMER 2

static int hookid = 0;
int counter = 0;

int get_counter(Timer_t* timer){
	return timer->counter;
}

void reset_counter(Timer_t* timer){
	timer->counter = 0;
	timer->boolean = 0;
}
...
void timer_int_handler(Timer_t* timer) {
	timer->counter++;
	timer->boolean = 1;
}
